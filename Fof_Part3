...
object FOFApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Step 6: Find shared friends of friends who are not direct friends between pairs of all users
    val allUsersRDD = outputRDD.map { case (person, _) => person }.distinct()

    val pairRDD = allUsersRDD.cartesian(allUsersRDD)
      .filter { case (personA, personB) => personA < personB } // Avoid duplicate pairs
      .map { case (personA, personB) =>
        val potentialFriendsA = broadcastFriendsMap.value.getOrElse(personA, List())
        val potentialFriendsB = broadcastFriendsMap.value.getOrElse(personB, List())
    ...
    // Step 7: Format the output for shared friends of friends between pairs of users
    val formattedPairRDD = pairRDD.map { case ((personA, personB), sharedCount) =>
      s"($personA, $personB) -> $sharedCount shared friends of friends"
    }

    // Step 8: Print the filtered pairs to the console
    formattedPairRDD.collect().foreach(println)

    // Step 9: Save the filtered pairs to HDFS
