...
object MutualFriendsApp {
  ...
  val filterOutputHDFS = "hdfs://localhost:9000/user/hthtd/OutputFolder_Part3"
  ...
  def main(args: Array[String]): Unit = {
    ...
    // New functionality: Filter users starting with "1" and mutual friends count ending with "0"
    val filteredUsersRDD = userMutualFriendsCountRDD.filter { case (user, count) =>
      user.startsWith("1") && count.toString.endsWith("0")  // Users starting with "1" and friends count ending with "0"
    }

    // Print filtered users to the console
    println("\nFiltered Users (Starting with '1' and Friends Count Ending with '0'):")
    filteredUsersRDD.collect().foreach { case (user, count) =>
      println(s"User $user: $count mutual friends")
    }

    // Save filtered users to HDFS
    filteredUsersRDD.map { case (user, count) =>
      s"User $user: $count mutual friends"
    }.coalesce(1).saveAsTextFile(filterOutputHDFS)
