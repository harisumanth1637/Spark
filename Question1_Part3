...
object TriadicClosureApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val outputHighMutualFriendsHDFS = "hdfs://localhost:9000/user/hthtd/Output_Part3"
    ...
    // Step 5: Check if the triadic closure is satisfied and find pairs where mutual friends > 50
    val triadicClosureResults = groupedPairsRDD.flatMap { case (pair, mutualFriends) =>
      ...
      // Filter for triadic closure not satisfied
      val result = if (!isDirectlyConnected) {
    ...
    // Step 6: Filter pairs with mutual friends greater than 50
    val highMutualFriendsResults = groupedPairsRDD.filter { case (_, mutualFriends) =>
      mutualFriends.size > 50
    }.map { case (pair, mutualFriends) =>
      s"$pair has ${mutualFriends.size} mutual friends"
    }

    // Step 7: Print the results to the console (if required)
    //triadicClosureResults.collect().foreach(println)

    // Step 8: Save the triadic closure output to HDFS
    sc.parallelize(triadicClosureResults.collect()).saveAsTextFile(outputHDFS)

    // Step 9: Save the high mutual friends output to HDFS
    highMutualFriendsResults.coalesce(1).saveAsTextFile(outputHighMutualFriendsHDFS)
